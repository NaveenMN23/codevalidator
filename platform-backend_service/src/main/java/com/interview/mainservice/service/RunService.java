...
public class RunService {
    ...
    public RunResponse run(UUID userId, UUID problemId, RunRequest request) {
        ...
        String sessionId = sessionId(userId, problemId);
        try {
    ...
    // Used verbatim as a directory name for the session's staging dir and inside Docker
    // volume bind-mount specs ("source:dest:mode") on the Execution Service side — must
    // not contain ':' (breaks bind-mount parsing) or '/' (breaks the directory name).
    private String sessionId(UUID userId, UUID problemId) {
